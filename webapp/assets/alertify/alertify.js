...
(function(global, undefined) {
	...
	Alertify = function() {
		...
		getTransitionEvent = function() {
			var t, type, supported = false, el = document
					.createElement("fakeelement"), transitions = {
		...
		_alertify = {
			...
			addListeners : function(fn) {
				...
				key = function(event) {
					...
					if ((keyCode === keys.SPACE && !hasInput)
			 ...
			 *            el HTML Object
			 * @param {Event}
			 *            event Event to attach to element
			 * @param {Function}
			...
			handleErrors : function() {
				if (typeof global.onerror !== "undefined") {
					...
					global.onerror = function(msg, url, line) {
						self.error("[" + msg + " on line " + line + " of "
								+ url + "]", 0);
					};
					return true;
				} else {
			 ...
			 * 
			 * @param {String}
			 *            secondary The secondary button HTML string
			 * @param {String}
			 *            primary The primary button HTML string
			 * 
			 * @return {String} The appended button HTML strings
			 */
			appendButtons : function(secondary, primary) {
				return this.buttonReverse ? primary + secondary : secondary
						+ primary;
			},
			 ...
			 * 
			 * @param {Object}
			 *            item Current object in the queue
			 * 
			...
			build : function(item) {
				...
				switch (type) {
				case "confirm":
					html = html.replace("{{buttons}}", this.appendButtons(
							dialogs.buttons.cancel, dialogs.buttons.ok));
					html = html.replace("{{ok}}", this.labels.ok).replace(
			 ...
			 * 
			 * @param {Object}
			 *            elem HTML Element of log message to close
			 * @param {Number}
			 *            wait [optional] Time (in ms) to wait before
			 *            automatically hiding the message, if 0 never hide
			 * 
			 * @return {undefined}
			 */
			 ...
			 * 
			 * @param {String}
			 *            message The message passed from the callee
			 * @param {String}
			 *            type Type of dialog to create
			 * @param {Function}
			 *            fn [Optional] Callback function
			 * @param {String}
			 *            placeholder [Optional] Default value for prompt input
			 *            field
			 * @param {String}
			 *            cssClass [Optional] Class(es) to append to dialog box
			 * 
			 * @return {Object}
			 */
			...
			hide : function() {
				...
				if (queue.length > 0)
					...
					isopen = false;
					// Hide the dialog box after transition
					// This ensure it doens't block any element from being
					// clicked
					transitionDone = function(event) {
						event.stopPropagation();
						// unbind event so function only gets called once
						self.unbind(elDialog, self.transition.type,
								transitionDone);
					};
					// whether CSS transition exists
					if (this.transition.supported) {
						this.bind(elDialog, this.transition.type,
								transitionDone);
			 ...
			 *            message The message passed from the callee
			 * @param {String}
			 *            type [Optional] Optional type of log message
			 * @param {Number}
			 *            wait [Optional] Time (in ms) to wait before
			 *            auto-hiding the log
			 * 
			...
			/**
			 * Add new log message If a type is passed, a class name
			 * "alertify-log-{type}" will get added. This allows for custom look
			 * and feel for various types of notifications.
			 * 
			 * @param {String}
			 *            message The message passed from the callee
			 * @param {String}
			 *            type [Optional] Type of log message
			 * @param {Number}
			 *            wait [Optional] Time (in ms) to wait before
			 *            auto-hiding
			 * 
			...
			notify : function(message, type, wait) {
				...
				log.className = "alertify-log"
						+ ((typeof type === "string" && type !== "") ? " alertify-log-"
								+ type
								: "");
			...
			setup : function(fromQueue) {
				...
				btnCancel = $("alertify-cancel") || undefined;
						btnFocus = (_alertify.buttonFocus === "cancel") ? btnCancel
								: ((_alertify.buttonFocus === "none") ? $("alertify-noneFocus")
										: btnOK), input = $("alertify-text")
								|| undefined;
			 ...
			 *            el HTML Object
			 * @param {Event}
			 *            event Event to detach to element
			 * @param {Function}
			 *            fn Callback function
			 * 
			 * @return {undefined}
